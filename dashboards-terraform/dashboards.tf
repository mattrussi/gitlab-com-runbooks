locals {
  ...
  dashboard_jsonnet_files         = fileset(local.dashboards_dir, "**/*.{json,dashboard.jsonnet}")
  shared_dashboards_jsonnet_files = fileset(local.dashboards_dir, "**/*.shared.jsonnet")
}

data "jsonnet_file" "dashboard" {
  for_each = setunion(local.dashboard_jsonnet_files, local.shared_dashboards_jsonnet_files)
...
locals {
  single_dashboards = {
    for dashboard in [
      for path in local.dashboard_jsonnet_files :
      {
