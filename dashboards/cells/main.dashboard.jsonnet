...
.addPanel(
  text.new(
    ...
    mode='markdown',
    content=|||
      [Observability based performance analysis](https://handbook.gitlab.com/handbook/engineering/infrastructure/cells/cells_test_strategy/#observability-testing) provides a view into how performance has been to help identify problems before they become customer facing problems.
...
.addPanels(layout.grid([
  ...
  ], cols=2, rowHeight=12, startRow=510)
)

.addPanel(
  row.new(title='Pingdom', collapse=true).addPanels(layout.grid([
    basic.gaugePanel(
    ...
    basic.timeseries(
      ...
      thresholds=[
          ...
          },
          {
            ...
            line: false,
            yaxis: 'left',
            fillColor: colorScheme.criticalColor,
          },
        ],
         format='ms',
        ).addSeriesOverride({
    ...
    ], cols=1, rowHeight=12, startRow=1100)
    ).addPanels(layout.grid([
      basic.timeseries(
    ...
    ], cols=2, rowHeight=12, startRow=1200)
  ),
  gridPos={
    x: 0,
    y: 1000,
...
.addPanel(
    row.new(title='CloudFlare', collapse=true).addPanels(layout.grid([
      basic.gaugePanel(
      ...
      basic.timeseries(
        ...
        format='µs',
        thresholds=[
              thresholds.errorLevel('gt', 5000),
              thresholds.optimalLevel('gt', 0),
        ],
      ),
      basic.timeseries(
      ...
      ], cols=2, rowHeight=12, startRow=2100)
  ),
  gridPos={
    x: 0,
    y: 2000,
