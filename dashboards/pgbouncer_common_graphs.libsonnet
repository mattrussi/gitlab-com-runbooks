  ...
  workloadStats(serviceType, startRow)::
    ...
    layout.grid([
      ...
      basic.timeseries(
        ...
        query=|||
          sum(
            rate(pgbouncer_stats_queries_duration_seconds_total{type="%(serviceType)s", environment="$environment"}[$__interval])
            or
            rate(pgbouncer_stats_queries_duration_seconds{type="%(serviceType)s", environment="$environment"}[$__interval])
          ) by (fqdn)
        ||| % formatConfig,
        legendFormat='{{ fqdn }}',
        format='ops',
      ...
      basic.timeseries(
        ...
        query=|||
          sum(
            rate(pgbouncer_stats_server_in_transaction_seconds_total{type="%(serviceType)s", environment="$environment"}[$__interval])
            or
            rate(pgbouncer_stats_server_in_transaction_seconds{type="%(serviceType)s", environment="$environment"}[$__interval])
          ) by (fqdn)
        ||| % formatConfig,
        legendFormat='{{ fqdn }}',
        format='s',
  ...
  connectionPoolingPanels(serviceType, startRow)::
    ...
    layout.grid([
      ...
      basic.latencyTimeseries(
        ...
        query=|||
          sum by (database, environment, type) ((
            rate(pgbouncer_stats_client_wait_seconds_total{%(nodeSelector)s, database!="pgbouncer"}[$__interval])
            or
            rate(pgbouncer_stats_client_wait_seconds{%(nodeSelector)s, database!="pgbouncer"}[$__interval])
          ) / on() group_left() %(WAIT_TIME_CORRECTION_FACTOR)s)
        ||| % formatConfig,
      ...
      basic.latencyTimeseries(
        ...
        query=|||
          sum by (database, environment, type, fqdn, job) ((
            rate(pgbouncer_stats_client_wait_seconds_total{%(nodeSelector)s, database!="pgbouncer"}[$__interval])
            or
            rate(pgbouncer_stats_client_wait_seconds{%(nodeSelector)s, database!="pgbouncer"}[$__interval])
          ) / on() group_left() %(WAIT_TIME_CORRECTION_FACTOR)s)
