...
.addPanels(
  layout.grid([
    ...
    .addTarget(
      promQuery.target(
        |||
          sum (registry_notifications_events_total{environment="$environment", cluster=~"$cluster", stage="$stage", exported_type="Successes"})
          /
...
.addPanels(
  layout.grid([
    basic.timeseries(
      title='Events per second (Aggregate)',
    ...
    graphPanel.new(
      'Pending events',
      description='The current number of events pending to be sent.',
      format='ops',
      linewidth=1,
      fill=0,
