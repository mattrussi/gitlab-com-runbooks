  ...
  layout.grid([
    ...
    .addTarget(
      promQuery.target(
        |||
          sum (
            rate(registry_notifications_events_total{environment="$environment", cluster=~"$cluster", stage="$stage", exported_type="Events"}[$__interval])
          )
        |||
      )
    ),
    ...
    .addTarget(
      promQuery.target(
        |||
          sum(
            max_over_time(registry_notifications_pending_total{environment="$environment", cluster=~"$cluster", stage="$stage"}[$__interval])
          )
        |||
      )
    ),
    ...
    .addTarget(
      promQuery.target(
        |||
          sum ( rate(registry_notifications_events_total{environment="$environment", cluster=~"$cluster", stage="$stage", exported_type="Successes"}[$__interval]))
          /
  ...
  layout.grid([
    ...
    .addTarget(
      promQuery.target(
        |||
          sum by (code) (
            rate(registry_notifications_status_total{environment="$environment", cluster=~"$cluster", stage="$stage"}[$__interval])
          )
        |||,
      )
    ),
