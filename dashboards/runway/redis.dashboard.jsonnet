...
local clientPanels(serviceType, startRow) =
  local formatConfig = {
...
local workload(serviceType, startRow) =
  local formatConfig = {
...
local data(serviceType, startRow) =
  local formatConfig = {
    selector: selectors.serializeHash({
      environment: '$environment',
      type: serviceType,
...
local redisDashboard(service) =
  ...
  .addPanels(data(serviceType=service, startRow=3000));


redisDashboard('redis-runway-managed').overviewTrailer()
