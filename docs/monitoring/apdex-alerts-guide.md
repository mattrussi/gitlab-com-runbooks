...
An apdex alert in #production should be paired with an alert in #production from [Slackline](https://gitlab.com/gitlab-com/gl-infra/slackline).  The Prometheus link on the origin alert might give you a broad brush idea of the impact (length, intensity etc), but you probably want to find the Slackline message.  That will also give you an idea of whether it's a slow or fast burn (see [Burn Rate](#burn-rate) and more particularly a '<service> Overview' button/link, which takes you to the relevant Grafana dashboard:
