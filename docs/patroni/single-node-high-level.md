...
## Analysis steps
1. Asses Postgres performance issue at a glance:

    Check the first panel "Active session history (ASH)". 

    *This panel shows number of wait events in time, basically it represents what database is busy with. It has reference line with number of vCPUs on a node.*
...
2.  Verify problem symptoms observed on the ASH panel by reviewing the Host Stats section:  
    - CPU: 
...
3. Check correlation between Postgres performance issues and Postgres stats:
    - **Latency**:
