...

{
  http(startRow)::
    layout.grid([
      ...
      basic.timeseries(
        ...
        query='sum(registry_http_in_flight_requests{cluster=~"$cluster", namespace="$namespace"}) by (method, route, code)',
  ...
  cache(startRow)::
    layout.grid([
      ...
      basic.singlestat(
        title='Hit %',
        query='sum(rate(registry_storage_cache_total{cluster=~"$cluster", environment="$environment", namespace="$namespace", exported_type="Hit"}[$__interval])) / sum(rate(registry_storage_cache_total{cluster=~"$cluster", environment="$environment", namespace="$namespace", exported_type="Request"}[$__interval]))',
        colors=[
  ...
  version(startRow)::
    layout.grid([
      basic.timeseries(
        title='Version',
        query='count(gitlab_build_info{app="registry", cluster=~"$cluster", environment="$environment", namespace="$namespace"}) by (cluster, version)',
