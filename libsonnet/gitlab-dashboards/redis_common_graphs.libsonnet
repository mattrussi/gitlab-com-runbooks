...
{
  clientPanels(serviceType, startRow, cluster=false, sharded=false, useTimeSeriesPlugin=false)::
    local formatConfig = {
  ...
  workload(serviceType, startRow, cluster=false, sharded=false, useTimeSeriesPlugin=false)::
    local formatConfig = {
  ...
  data(serviceType, startRow, hitRatio=false, cluster=false, sharded=false, useTimeSeriesPlugin=false)::
    local formatConfig = {
      selector: selectors.serializeHash({
        environment: '$environment',
        type: serviceType,
  ...
  replication(serviceType, startRow, cluster=false, sharded=false, useTimeSeriesPlugin=false)::
    local formatConfig = {
      selector: selectors.serializeHash({
        environment: '$environment',
        type: serviceType,
  ...
  cluster(serviceType, startRow, useTimeSeriesPlugin=false)::
    local formatConfig = {
      serviceType: serviceType,
    };
  ...
  redisDashboard(service, cluster=false, hitRatio=false, sharded=false, useTimeSeriesPlugin=false)::
    local dashboard =
      serviceDashboard.overview(service)
      .addPanels(self.clientPanels(serviceType=service, startRow=1000, cluster=cluster, sharded=sharded, useTimeSeriesPlugin=useTimeSeriesPlugin))
      .addPanels(self.workload(serviceType=service, startRow=2000, cluster=cluster, sharded=sharded, useTimeSeriesPlugin=useTimeSeriesPlugin))
      .addPanels(self.data(serviceType=service, startRow=3000, hitRatio=hitRatio, cluster=cluster, sharded=sharded, useTimeSeriesPlugin=useTimeSeriesPlugin))
      .addPanels(self.replication(serviceType=service, startRow=4000, cluster=cluster, sharded=sharded, useTimeSeriesPlugin=useTimeSeriesPlugin));
    ...
    if cluster then
      templatedDashboard.addPanels(self.cluster(serviceType=service, startRow=5000, useTimeSeriesPlugin=useTimeSeriesPlugin))
    else
      templatedDashboard
      .addPanels(
        layout.titleRowWithPanels(
