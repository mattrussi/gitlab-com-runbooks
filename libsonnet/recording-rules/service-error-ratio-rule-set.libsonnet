  ...
  serviceErrorRatioRuleSet(suffix)::
    {
      generateRecordingRules()::
        ...
        [{
          ...
          expr: |||
            sum by (env,environment,tier,type,stage) (
              gitlab_component_ops:rate%(suffix)s{monitor!="global"} >= 0 and on(component, type, environment) (gitlab_component_service:mapping{monitor!="global", aggregate_rps="yes"})
            )
          ||| % format,
        }, {
