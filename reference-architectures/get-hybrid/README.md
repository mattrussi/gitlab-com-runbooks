...
| `go_memory` | `gitaly`, `praefect` | Go's memory allocation strategy can make it look like a Go process is saturating memory when measured using `go_memstat_alloc_bytes`, as this includes the heap, which isnt actually used memory and can be freely allocated by the OS (but it's currently being held by go).  | ✅ | 98% |
