...
# Storage module
module Storage
  NODE_CONFIGURATION = (defined?(::Gitlab) ? ::Gitlab.config.repositories.storages.dup : {}).freeze
...
module Storage
  ...
  module CommandLineSupport
    ...
    class Options
      ...
      def define_options
        @parser.banner = "Usage: #{$PROGRAM_NAME} [options] --current-file-server <servername> --target-file-server <servername>"
        @parser.separator ''
      ...
      def define_head
        @parser.on_head('--current-file-server=<SERVERNAME>', 'Source storage node server') do |server|
          @options[:current_file_server] = server
        end
        @parser.on_head('--target-file-server=<SERVERNAME>', 'Destination storage node server') do |server|
      ...
      def define_projects_option
        @parser.on('--projects=<project_id,...>', Array, 'Select specific projects to migrate') do |project_identifiers|
          @options[:projects] ||= []
          unless project_identifiers.respond_to?(:all?) && project_identifiers.all? { |s| resembles_integer? s }
      ...
      def define_csv_option
        @parser.on('--csv=<file_path>', 'Absolute path to CSV file enumerating projects ids') do |file_path|
      ...
      def define_wait_option
        description = "Timeout in seconds for migration completion; default: #{@options[:timeout]}"
...
module Storage
  ...
  class Rebalancer
    ...
    def configure_project_migration_logging
      fields = { date: Time.now.strftime(::Storage::MIGRATION_TIMESTAMP_FORMAT) }
    ...
    def get_commit_id(project_id)
      ...
      log.debug "Response payload sample: #{JSON.pretty_generate(payload)}"
      log.info "Total commits: #{payload.length}"
    ...
    def get_project_ids(project_ids = [], opts = { limit: 0 })
      clauses = namespace_filter(@options[:clauses].dup, @options[:group])
      clauses.merge!(repository_storage: @options[:current_file_server])
    ...
    def rebalance
      ...
      if !project_identifiers.empty?
        project_identifiers = get_project_ids(project_identifiers, limit: project_identifiers.length)
...
module Script
  ...
  def main(args = parse(ARGV, ARGF))
    ...
    if gitaly_source_address == gitaly_destination_address
      message = 'Given destination git storage node must not have the same gitaly address as ' \
        'the source'
      raise message
    end
    ...
    if args[:list_nodes]
      ::Storage::NODE_CONFIGURATION.keys.uniq.sort.each do |repository_storage_node|
        gitaly_address = ::Storage::NODE_CONFIGURATION[repository_storage_node]['gitaly_address']
