    ...
    class OptionsParser
      ...
      def define_skip_option
        ...
        @parser.on('--skip=<project_id,...>', Array, description) do |project_identifiers|
          ...
          if project_identifiers.respond_to?(:all?) && project_identifiers.all? { |s| resembles_integer? s }
            @options[:excluded_projects].concat project_identifiers.map(&:to_i).delete_if { |i| i <= 0 }
          else
  ...
  class Rebalancer
    ...
    def print_node_list
      ::Storage.node_configuration.keys.uniq.sort.each do |repository_storage_node|
        storage_node_config = ::Storage.node_configuration.fetch(repository_storage_node, {})
        gitaly_address = storage_node_config['gitaly_address']
    ...
    def move_projects(project_ids, min_amount = @options[:move_amount])
      log.debug "Moving #{project_ids.length} projects..."
      total = 0
