...
To modify the service catalog format, edit [schema](service-catalog.schema.json) then run `make generate`. Additional properties are disabled by default, please add new properties sparingly. For dynamic data, consider linking to single source of truth instead.
...
The service catalog uses [jsonschema2md](https://github.com/adobe/jsonschema2md) for schema documentation generation. After editing schema, run the following command:

```
make generate-service-catalog-schema-docs
```

After successful generation, you can view autogenerated static [documentation](docs/service-catalog.md).
