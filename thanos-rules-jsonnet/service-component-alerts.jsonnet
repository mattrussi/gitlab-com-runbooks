...
local errorRule(metricSelectorHash) = {
...
local apdexRulePaging = apdexRule({ monitor: 'global', component: { nre: std.join('|', nonPagingComponents) } }) + {
  labels+: {
    severity: 's2',
    pager: 'pagerduty',
  },
};
local apdexRuleNonPaging = apdexRule({ monitor: 'global', component: { re: std.join('|', nonPagingComponents) } }) + {
  labels+: {
    severity: 's3',
    pager: 'issue',
...
local errorRulePaging = errorRule({ monitor: 'global', component: { nre: std.join('|', nonPagingComponents) } }) + {
  labels+: {
    severity: 's2',
    pager: 'pagerduty',
  },
};
local errorRuleNonPaging = errorRule({ monitor: 'global', component: { re: std.join('|', nonPagingComponents) } }) + {
  labels+: {
    severity: 's3',
    pager: 'issue',
    project: 'gitlab.com/gitlab-com/gl-infra/production',
...
local rules = {
  groups: [{
    ...
    interval: '1m',
    rules: [
      alerts.processAlertRule(apdexRulePaging),
      alerts.processAlertRule(apdexRuleNonPaging),
      alerts.processAlertRule(errorRulePaging),
      alerts.processAlertRule(errorRuleNonPaging),
    ],
  }],
};
